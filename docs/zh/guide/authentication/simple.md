...
> [!WARNING]
> Dozzle Cloud 同样是实例级别的。关联操作会保存一个 API key，把警报派发、日志推送和工具执行统统指向某一个云账号。拥有 `cloud` 角色的用户可以把实例关联到自己的云账号，从而看到所有容器，也可以解除已有的关联。只把这个角色授予你信任其访问实例上所有容器的用户。
>
> 这个角色管的是关联，而不是查看。任何已登录的用户都可以搜索云端日志、查看云端警报，范围仅限于各自的过滤器。而由云端自己发起的工具调用，比如在 Telegram 或 Discord 里提的问题，背后并没有对应的 Dozzle 用户，因此使用的是实例过滤器。
