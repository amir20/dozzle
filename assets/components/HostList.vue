...
setTimeout(() => {
  watchEffect(() => {
    Object.values(hosts.value).forEach((host) => {
      console.log("cpu", Math.floor((weightedStats[host.id].weighted.totalCPU / (host.nCPU * 100)) * 100));
      console.log("mem", Math.floor((weightedStats[host.id].weighted.totalMem / host.memTotal) * 100));
    });
  });
}, 500);
</script>
